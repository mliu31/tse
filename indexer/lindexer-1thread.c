...
void* read_document_wbw(void *threadID) {
	...
	while(true) {
		...
		pthread_mutex_lock(tid->hashtable->mutex);
		loadedpage = pageload(tid->page_id, pagedirectory);
		id = tid->page_id;
		(tid->page_id)++;
		pthread_mutex_unlock(tid->hashtable->mutex);
		...
		while(true) {
			...
			if(NormalizeWord(&word) == 0) {
				indexentry = lhsearch(tid->hashtable, search, word, strlen(word));
				if(indexentry == NULL) {
					indexentry = makeindexentry(word, id);
					lhput(tid->hashtable, indexentry, word, strlen(word));
				} else {
...
int main(int argc, char *argv[]) {
	...
	if(argc != 4) { 
		printf("usage: lindexer <pagedir> <indexfilenm> <no_of_threads>\n"); 
		return -1;
	}
